...
* Para testes locais da integração com Webhooks do GitHub antes do deploy da API, é utilizada a ferramenta **Ngrok** (configurável no arquivo `mobile/src/config.ts`) para expor a porta local (`localhost:8080`) para a internet pública, permitindo a entrega dos *payloads* em ambiente de desenvolvimento.
