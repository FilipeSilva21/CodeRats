...
export default function SquadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { currentSquad, members, isLoading, error, fetchSquadDetails, leaveSquad } = useSquadStore();
      ...
      <main className="flex flex-col flex-1 p-8 items-center justify-center">
        <div className="bg-red-500/10 border border-red-500/30 p-8 rounded-2xl max-w-lg w-full text-center">
          ...
          <button
            ...
            className="mt-6 bg-cr-bg border border-cr-border px-6 py-2 rounded-xl text-cr-text-bold hover:bg-cr-surface transition-colors"
          >
            Go Back
          </button>
        </div>
    ...
    <main className="flex flex-col flex-1 p-8 overflow-y-auto">
      <div className="max-w-4xl w-full mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4">
          <button
            onClick={() => router.push('/squad')}
            className="w-10 h-10 flex items-center justify-center bg-cr-surface border border-cr-border rounded-full text-cr-text hover:bg-cr-bg transition-colors"
          >
            ←
          </button>
          ...
          <div className="h-32 bg-gradient-to-r from-indigo-900/50 to-cr-surface relative">
            <div className="absolute -bottom-10 left-8">
              {currentSquad.imageUrl ? (
                <img src={currentSquad.imageUrl} alt={currentSquad.name} className="w-24 h-24 rounded-2xl border-4 border-cr-surface bg-cr-bg" />
              ) : (
                <div className="w-24 h-24 rounded-2xl border-4 border-cr-surface bg-cr-bg flex items-center justify-center text-4xl font-black text-cr-text-subtle shadow-lg">
            ...
            <div className="absolute top-6 right-6">
              <button
                ...
                className="flex items-center gap-2 bg-cr-bg/80 backdrop-blur-sm border border-cr-border px-4 py-2 rounded-xl hover:bg-cr-bg transition-colors group"
              >
        ...
        <div className="flex items-center justify-between mt-8 mb-4">
          ...
          <button
            ...
            disabled={isLeaving}
            className="flex items-center gap-2 bg-red-500/10 text-red-500 hover:bg-red-500/20 px-4 py-2 rounded-xl font-bold transition-colors text-sm"
          >
            ...
            {members.map((member, index) => (
              <div key={member.userId} className="flex items-center justify-between p-5 hover:bg-cr-bg transition-colors">
                <div className="flex items-center gap-5">
                  <div className={`w-8 font-black text-center ${index === 0 ? 'text-yellow-500 text-xl' :
                  ...
                  {member.avatarUrl ? (
                    ...
                    <div className="w-12 h-12 rounded-full bg-cr-bg border border-cr-border flex items-center justify-center font-bold text-cr-text-muted text-lg">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {member.userId === currentSquad.ownerId && (
                        <span className="text-[10px] font-black bg-amber-500/20 text-amber-500 px-2 py-0.5 rounded uppercase tracking-wider">Owner</span>
                      )}
                      {member.userId === user?.id && (
                        <span className="text-[10px] font-black bg-[#58a6ff]/20 text-[#58a6ff] px-2 py-0.5 rounded uppercase tracking-wider">You</span>
                      )}
                    </div>
